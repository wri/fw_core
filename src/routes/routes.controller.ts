...
export class RoutesController {
  ...
  @Post('/sync')
  async create(@Req() request: Request, @Body() routesToSync: CreateRouteDto[]) {

    let syncedRoutes = []

    for await (const route of routesToSync) {

      const existingRoute = await this.routesService.findOne({routeId: route.id});
...
/*   @Patch(':id')
  ...
  async remove(@Req() request: Request, @Param('id') id: string): Promise<void> {
    ...
    if(!route) throw new HttpException("This route doesn't exist", HttpStatus.NOT_FOUND);
    // check user is manager of route team
    const managedTeams = await this.teamsService.findAllManagedTeams(request.user.id);
    if(!managedTeams.includes(route.teamId) && route.createdBy !== request.user.id) throw new HttpException("You cannot delete this route", HttpStatus.FORBIDDEN)
