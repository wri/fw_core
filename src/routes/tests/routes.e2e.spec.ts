...
import assignments from './routes.constants'
...
describe('Routes', () => {
  ...
  describe('POST /routes/sync', () => {
    ...
    it('should create routes', async () => {
      ...
      const createdRoutes = await formsDbConnection.collection('routes').count({createdBy: ROLES.USER.id})
    ...
    it('should return created routes', async () => {
      ...
      expect(response.body.data[1].attributes.locations[0]).toHaveProperty('accuracy', 5);
    ...
    it('should add createdBy, routeId, active fields', async () => {
      ...
      expect(response.body.data.length).toBe(2);
      expect(response.body.data[0]).toHaveProperty('attributes');
      expect(response.body.data[0].attributes).toHaveProperty('createdBy', ROLES.USER.id);
      expect(response.body.data[1]).toHaveProperty('attributes');
      expect(response.body.data[1].attributes).toHaveProperty('routeId', "cea34015-bfaf-46c2-a660-db1e9819b516");
      ...
      const route1 = await formsDbConnection.collection('routes').findOne({_id: new mongoose.Types.ObjectId(response.body.data[0].id)});
      expect(route1).toBeDefined();
      expect(route1).toHaveProperty('active', true);
      expect(route1).toHaveProperty('routeId', "cea34015-bfaf-46c2-a660-db1e9819b515");
      expect(route1).not.toHaveProperty('id')

    });

    it('should only sync and return unsynced routes', async () => {
      const existingRoute = await formsDbConnection.collection('routes').insertOne({
        ...routeConstants.defaultRoute,
      ...
      expect(response.body.data[0]).toHaveProperty('attributes');
      expect(response.body.data[0].attributes).toHaveProperty('routeId', "cea34015-bfaf-46c2-a660-db1e9819b516");
    });
  });
  ...
  describe('GET /routes/user', () => {
    ...
    it('should return an array of users active routes', async () => {
      ...
      expect(response.body).toHaveProperty('data');
      expect(response.body.data.length).toBe(2);
      expect(response.body.data[0]).toHaveProperty('id', route1.insertedId.toString());
      expect(response.body.data[0]).toHaveProperty('attributes');
      expect(response.body.data[0].attributes).toHaveProperty('routeId', "cea34015-bfaf-46c2-a660-db1e9819b515");
      expect(response.body.data[1]).toHaveProperty('id', route3.insertedId.toString());
      expect(response.body.data[1]).toHaveProperty('attributes');
      expect(response.body.data[1].attributes).toHaveProperty('routeId', "cea34015-bfaf-46c2-a660-db1e9819b517");
    });
  });
  ...
  describe('GET /routes/:id', () => {
    ...
    it('should return a route', async () => {
      ...
      expect(response.body).toHaveProperty('data');
      expect(response.body.data).toHaveProperty('id', route1.insertedId.toString());
      expect(response.body.data).toHaveProperty('attributes');
      expect(response.body.data.attributes).toHaveProperty('createdBy', ROLES.USER.id);
  ...
  describe('DELETE /routes/:id', () => {
    ...
    it('should deactivate a route (change "active" field from true to false)', async () => {
      ...
      const deactivated = await formsDbConnection.collection('routes').findOne({routeId: "cea34015-bfaf-46c2-a660-db1e9819b515"});
    ...
    it('should allow a team manager to deactivate a team route', async () => {
      ...
      const deactivated = await formsDbConnection.collection('routes').findOne({_id: route1.insertedId});
      expect(deactivated).toBeDefined();
      expect(deactivated).toHaveProperty('active', false);
    });
