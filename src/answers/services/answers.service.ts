...
import { Answer, AnswerDocument, IAnswer, IAnswerResponse } from '../models/answer.model';
...
export class AnswersService {
  ...
  async getAllTemplateAnswers({ template, user }: { template: TemplateDocument, user: IUser }): Promise<IAnswer[]> {
    let filter = {};
    const confirmedUsers = [];
    // add current user to users array
    confirmedUsers.push(user.id);

    let teamsManaged = await this.teamsService.findAllManagedTeams(user.id)
    // get all managed teams users
    for await (const team of teamsManaged) {
      // get users of each team
      const users = await this.teamMembersService.findAllTeamMembers(team, EMemberRole.Administrator);
    ...
    if (user.role === "ADMIN" || user.id === template.user) {
      ...
      filter = {
        $and: [{ report: template._id }, { user: new mongoose.Types.ObjectId(user.id) }]
      };
    }
  ...
  async filterAnswersByArea({ 
    ...
    // get all area teams
    const areaTeamsRelations = await this.teamAreaRelationService.find({ areaId });
    ...
    let filter = {
      $and: [{ report: new mongoose.Types.ObjectId(reportId) }, { user: { $in: userIds } }, { areaOfInterest: areaId }]
    };
  ...
  async addUsernameToAnswers(answers: AnswerDocument[]): Promise<AnswerDocument[]> {

    for await (let answer of answers) answer = await this.addUsernameToAnswer(answer);
    return answers
  };
}
