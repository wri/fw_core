import { Controller, Get, Post, Body, Patch, Param, Delete, Req, HttpException, HttpStatus, UseInterceptors, UploadedFiles } from '@nestjs/common';
...
export class AnswersController {
  ...
  async create(@UploadedFiles() fileArray: Array<Express.Multer.File>, @Body() fields: CreateAnswerDto, @Req() request: Request) {
    ...
    try {
      ...
      throw new HttpException(`Position values must be separated by ','`, HttpStatus.BAD_REQUEST);
    }
    ...
    const pushError = question => {
      throw new HttpException(`${question.label[answer.language]} (${question.name}) required`, HttpStatus.BAD_REQUEST);
    };
    const { questions } = template;
    if (!questions || (questions && !questions.length)) {
      throw new HttpException(`No question associated with this report`, HttpStatus.BAD_REQUEST);
    }
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (response && response.path && response.filename && question.type === "blob") {
        // upload file
        response = await this.s3Service.uploadFile(response.path, response.filename);
      }
  ...
  @Get('/area/:areaId')
  async getAreaAnswers(@Req() request: Request, @Param('areaId') areaId: string) {
    ...
    if (!answers) throw new HttpException("no answers found with current permissions", HttpStatus.NOT_FOUND)
  ...
  async findAll(@Req() request: Request) {
    const { user, template } = request;
    return { data: serializeAnswers(await this.answersService.getAllTemplateAnswers({ user, template })) }
  }
  ...
  async findOne(@Param('id') id: string, @Req() request: Request) {
    ...
    for await (const team of userTeams) {
      ...
      const users = await this.teamMembersService.findAllTeamMembers(team.id, EMemberRole.Administrator);
    ...
    if (!answer) throw new HttpException('No answer found with your permissions', HttpStatus.NOT_FOUND)

    return { data: serializeAnswers(await this.answersService.addUsernameToAnswer(answer)) };
  }
  ...
  async remove(@Param('id') id: string, @Req() request: Request): Promise<void> {
    ...
    let permitted = false;
    // get the answer
    const answer = await this.answersService.findOne({ '_id': new mongoose.Types.ObjectId(id) });
    if (answer.user.toString() === user.id.toString()) permitted = true;
    else {
      // get associated teams of answer area
      const areaTeams = await this.teamAreaRelationService.find({ areaId: answer.areaOfInterest });
    ...
    if (!permitted) throw new HttpException("You are not authorised to delete this record", 401)
