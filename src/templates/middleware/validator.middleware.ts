...
export class CreateTemplateMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    if(!body.name) throw new HttpException("Request body is missing field 'name'", HttpStatus.BAD_REQUEST);
    if(!body.questions) throw new HttpException("Request body is missing field 'questions'", HttpStatus.BAD_REQUEST);
    if(!body.languages) throw new HttpException("Request body is missing field 'languages'", HttpStatus.BAD_REQUEST);
    if(!body.defaultLanguage) throw new HttpException("Request body is missing field 'defaultLanguage'", HttpStatus.BAD_REQUEST);
    if(!body.status) throw new HttpException("Request body is missing field 'status'", HttpStatus.BAD_REQUEST);
    if(!["published", "unpublished"].includes(body.status)) throw new HttpException("Status must be published or unpublished", HttpStatus.BAD_REQUEST);
