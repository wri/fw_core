...
describe('Team Members', () => {
  ...
  describe('POST /teams/:teamId/users', () => {
    ...
    it('should return a 403 if a team monitor', async () => {
      const team1 = await teamsDbConnection.collection('gfwteams').insertOne({name: 'Test'});
      await teamsDbConnection.collection('teamuserrelations').insertOne({teamId: team1.insertedId.toString(), userId: ROLES.USER.id, email: ROLES.USER.email, status: EMemberStatus.Invited, role: EMemberRole.Manager})
      return await request(app.getHttpServer())
      .post(`/teams/${team1.insertedId.toString()}/users`)
      .set('Authorization', 'USER')
      .expect(403)
    ...
    it('should add a team member record', async () => {
      ...
      const teamMember = await teamsDbConnection.collection('teamuserrelations').findOne({
        teamId: team1.insertedId.toString(),
        userId: ROLES.ADMIN.id,
        email: ROLES.ADMIN.email,
        role: EMemberRole.Monitor
  ...
  describe('PATCH /teams/:teamId/users/reassignAdmin/:userId', () => {
    ...
    it('should make user the team admin and current admin manager', async () => {
      ...
      const newAdmin = await teamsDbConnection.collection('teamuserrelations').findOne({'_id': teamUser.insertedId});
      expect(newAdmin.role).toEqual(EMemberRole.Administrator);
      const oldAdmin = await teamsDbConnection.collection('teamuserrelations').findOne({'_id': adminUser.insertedId});
  ...
  describe('PATCH /teams/:teamId/users/:memberId', () => {
    ...
    it('should update user role', async () => {
      ...
      const updatedMember = await teamsDbConnection.collection('teamuserrelations').findOne({'_id': manager.insertedId})
  ...
  describe('DELETE /teams/:teamId/users/:memberId', () => {
    ...
    it('should delete the team member record', async () => {
      ...
      const member = await teamsDbConnection.collection('teamuserrelations').findOne({'_id': manager.insertedId});
