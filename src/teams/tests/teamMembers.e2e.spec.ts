...
describe('Team Members', () => {
  ...
  describe('GET /teams/:teamId/users', () => {
    ...
    it('should return an array of team members', async () => {
      ...
      await teamsDbConnection.collection('teamuserrelations').insertOne({
        ...
        userId: new mongoose.Types.ObjectId(ROLES.ADMIN.id),
      ...
      expect(response.body.data[0]).toHaveProperty(
        'id',
        member1.insertedId.toString(),
      );
      expect(response.body.data[1]).toHaveProperty(
        'id',
        member2.insertedId.toString(),
      );
    });
  });
  ...
  describe('POST /teams/:teamId/users', () => {
    ...
    it('should return a 403 if a team manager', async () => {
      const team1 = await teamsDbConnection
        .collection('gfwteams')
        .insertOne({ name: 'Test' });
      await teamsDbConnection.collection('teamuserrelations').insertOne({
    ...
    it('should add a team member record', async () => {
      ...
      const teamMember = await teamsDbConnection
        .collection('teamuserrelations')
        .findOne({
          teamId: team1.insertedId.toString(),
          userId: ROLES.ADMIN.id,
  ...
  describe('PATCH /teams/:teamId/users/reassignAdmin/:userId', () => {
    ...
    it('should make user the team admin and current admin manager', async () => {
      const team1 = await teamsDbConnection
        .collection('gfwteams')
        .insertOne({ name: 'Test' });
      const adminUser = await teamsDbConnection
        .collection('teamuserrelations')
        .insertOne({
          teamId: team1.insertedId,
          userId: new mongoose.Types.ObjectId(ROLES.USER.id),
      ...
      const teamUser = await teamsDbConnection
        .collection('teamuserrelations')
        .insertOne({
          teamId: team1.insertedId,
          userId: new mongoose.Types.ObjectId(ROLES.MANAGER.id),
  ...
  describe('PATCH /teams/:teamId/users/:memberId', () => {
    ...
    it('should succeed if a team manager', async () => {
      ...
      return await request(app.getHttpServer())
        .patch(
          `/teams/${team1.insertedId.toString()}/users/${member2.insertedId.toString()}`,
        )
    ...
    it('should succeed if a team admin', async () => {
      ...
      return await request(app.getHttpServer())
        .patch(
          `/teams/${team1.insertedId.toString()}/users/${manager.insertedId.toString()}`,
        )
        .send({ role: EMemberRole.Monitor })
        .set('Authorization', 'USER')
        .expect(200);
    ...
    it('should fail if updating team admin', async () => {
      ...
      return await request(app.getHttpServer())
        .patch(
          `/teams/${team1.insertedId.toString()}/users/${admin.insertedId.toString()}`,
        )
        .send({ role: EMemberRole.Monitor })
        .set('Authorization', 'USER')
        .expect(400);
  ...
  describe('DELETE /teams/:teamId/users/:memberId', () => {
    ...
    it('should succeed if a monitor deletes themselves', async () => {
      ...
      return await request(app.getHttpServer())
        .delete(
          `/teams/${team1.insertedId.toString()}/users/${member.insertedId.toString()}`,
        )
        .set('Authorization', 'USER')
        .expect(200);
    });
