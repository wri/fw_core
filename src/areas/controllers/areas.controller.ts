import { Controller, Get, Post, Body, Patch, Param, Delete, UseInterceptors, UploadedFile, Req, Logger, HttpException, HttpStatus } from '@nestjs/common';
...
export class AreasController {
  ...
  async getUserAreas(@Req() request: Request): Promise<IAreaResponse> {
    ...
    if (user && user.id) {
      try {
        ...
        areas.forEach(area => {
          if (area.attributes.userId !== user.id) throw new HttpException('Incorrect areas found', HttpStatus.SERVICE_UNAVAILABLE);
        });
        try {
          ...
          throw new HttpException("Error while retrieving area's geostore, template, and coverage", error.status)
        }
      } catch (error: any) {
  ...
  async createArea(@UploadedFile() image: Express.Multer.File, @Req() request: Request, @Body() body: CreateAreaDto): Promise<IAreaResponse> {
    ...
    if (user && user.id) {
      try {
        const { area, geostore, coverage } = await this.areasService.createAreaWithGeostore(
          { name, image },
          JSON.parse(geojson),
          user
        );
        ...
        try {
          [ data ] = await this.responseService.buildAreasResponse([ area ], { geostoreObj: geostore, coverageObj: coverage }, user);
        } catch (error) {
  ...
  async findOneArea(@Req() request: Request, @Param('id') id: string): Promise<IAreaResponse> {
    ...
    const areaTeams: string[] = await this.teamAreaRelationService.getAllTeamsForArea(id); // get list of teams associated with area
  ...
  @UseInterceptors(FileInterceptor('image', { dest: './tmp' }))
  async updateArea(@UploadedFile() image: Express.Multer.File, @Param('id') id: string, @Req() request: Request, @Body() updateAreaDto: UpdateAreaDto): Promise<IAreaResponse> {
    const { user }: {user: IUser} = request;
    // get the area
    ...
    if (user && user.id) {
      try {
        const { area, geostoreId, coverage } = await this.areasService.updateAreaWithGeostore(
          {
            name,
            image
          },
        ...
        try {
          [ data ] = await this.responseService.buildAreasResponse([area], {
            coverage
          }, user);
        } catch (e: any) {
          this.logger.error(e);
          throw new HttpException("Error while building area response", e.status);
        }
      } catch (e: any) {
        this.logger.error(e);
  ...
  @Delete('/:id')
  async deleteOneArea(@Param('id') id: string, @Req() request: Request): Promise<string> {
    const { user }: {user: IUser} = request;
    const area = await this.areasService.getArea(id, user);
    
    if(area.attributes.userId.toString() !== user.id.toString())
      throw new HttpException("You are not authorised to delete this record", HttpStatus.UNAUTHORIZED);
